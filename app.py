    ...
    with t_add:
        ...
        with st.form("admin_add_i", clear_on_submit=True):
            if selected_cat == "السجائر":
                st.warning("📋 تعليمات تسجيل السجائر: أدخل البيانات بالعلبة، وحدد سعر السيجارة الفردي")
            ...
            if st.form_submit_button("➕ تنفيذ الإضافة"):
                if n:
                    ...
                    auto_save(); st.success(f"✅ تم إضافة {n} لقسم {selected_cat}"); st.rerun()
