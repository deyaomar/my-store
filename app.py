...
if 'logged_in' not in st.session_state:
    ...
    if menu == "🛒 نقطة البيع":
        ...
        if not st.session_state.show_customer_form:
            ...
            if st.button("✅ تأكيد البيع وحفظ البيانات"):
                bid = str(uuid.uuid4())[:8]
                now_ts = datetime.now().strftime("%Y-%m-%d %H:%M")
                for e in st.session_state.current_bill_items:
                    st.session_state.inventory[e["item"]]["كمية"] -= e["qty"]
                    new_s = {'date': now_ts, 'item': e['item'], 'amount': e['amount'], 'profit': e['profit'], 'method': e['method'], 'customer_name': c_n, 'customer_phone': c_p, 'bill_id': bid}
    ...
    # --- 📦 المخزن والجرد ---
        ...
        tab1, tab2, tab3 = st.tabs(["📋 الرصيد الاحترافي", "⚖️ الجرد والمطابقة", "🗑️ التالف"])
        with tab1:
            cols = st.columns(3)
            for idx, (it, data) in enumerate(st.session_state.inventory.items()):
                is_low = data['كمية'] < 5
                with cols[idx % 3]:
                    st.markdown(f'<div class="stock-card {"low-stock" if is_low else ""}"><b>{it}</b><br>{format_num(data["كمية"])} كجم<br><small>شراء: {data["شراء"]} | بيع: {data["بيع"]}</small></div>', unsafe_allow_html=True)
        with tab2:
            st.info("نظام المطابقة السريع")
            for it, data in st.session_state.inventory.items():
                c1, c2, c3 = st.columns([2, 1, 2])
                c1.write(f"**{it}** (النظام: {format_num(data['كمية'])})")
                act = c2.text_input("الفعلية", key=f"aud_{it}")
                if act:
                    diff = clean_num(act) - data['كمية']
                    c3.write(f"الفرق: {format_num(diff)} | خسارة: {format_num(diff * data['شراء'])} ₪")

    # --- 📊 التقارير المالية (تم إضافة سجل الزبائن هنا) ---
        ...
        total_profit = st.session_state.sales_df['profit'].sum() - st.session_state.expenses_df['amount'].sum() - st.session_state.waste_df['loss_value'].sum()
        c1.markdown(f"<div class='report-card'><h3>💰 مبيعات اليوم</h3><h2>{format_num(daily_sales)} ₪</h2></div>", unsafe_allow_html=True)
        c2.markdown(f"<div class='report-card'><h3>💵 صافي الأرباح</h3><h2>{format_num(total_profit)} ₪</h2></div>", unsafe_allow_html=True)
        c3.markdown(f"<div class='report-card'><h3>📦 قيمة المخزن</h3><h2>{format_num(sum(v['كمية']*v['شراء'] for v in st.session_state.inventory.values()))} ₪</h2></div>", unsafe_allow_html=True)

        st.divider()
        
        # --- قسم سجل الزبائن الجديد ---
        st.subheader("👥 سجل مبيعات الزبائن اليومي")
        selected_date = st.date_input("اختر التاريخ لعرض السجل", datetime.now())
        date_str = selected_date.strftime('%Y-%m-%d')
        
        customer_day_df = st.session_state.sales_df[st.session_state.sales_df['date_only'] == date_str].copy()
        
        if not customer_day_df.empty:
            # إعادة تسمية الأعمدة للعرض فقط
            display_df = customer_day_df[['date', 'customer_name', 'customer_phone', 'item', 'amount', 'method']].rename(columns={
                'date': 'الوقت والتاريخ',
            ...
            st.info(f"لا توجد مبيعات مسجلة بتاريخ {date_str}")
        ...
        with st.form("add"):
            ...
            if st.form_submit_button("حفظ"):
                st.session_state.inventory[n] = {"قسم": "خضار", "شراء": clean_num(b), "بيع": clean_num(s), "كمية": clean_num(q)}
                auto_save(); st.rerun()
