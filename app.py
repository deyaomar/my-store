...
# 3. التصميم (CSS المطور للقائمة الجانبية والكروت)
st.markdown("""
    <style>
    ...
    [data-testid="stSidebar"] .stRadio div label:hover { background-color: #334155; }
...
# 5. القائمة الجانبية (تنسيق أبو عمر المطور)
...
# --- محتوى الصفحات (التقارير المالية) ---
...
# --- محتوى الصفحات (نقطة البيع) ---
    ...
    st.markdown("<h1 class='main-title'>🏪 إدارة محلات النظام</h1>", unsafe_allow_html=True)
    with st.form("br_add"):
    ...
    st.markdown("<h1 class='main-title'>💸 سجل المصاريف اليومية</h1>", unsafe_allow_html=True)
    with st.form("exp_f"):
    ...
    st.markdown("<h1 class='main-title'>👤 الملف الشخصي</h1>", unsafe_allow_html=True)
    st.write(f"المستخدم النشط: {st.session_state.active_user}")
