...
st.markdown("""
    ...
    .metric-card { background-color: white; padding: 20px; border-radius: 15px; box-shadow: 2px 2px 10px rgba(0,0,0,0.1); border-right: 5px solid #1e4d2b; margin-bottom: 20px; }
...
if 'logged_in' not in st.session_state:
    ...
    if 'inventory' not in st.session_state:
        st.session_state.inventory = {
            ...
            "كلمنتينا": {"كمية": 22.4, "شراء": 4.0, "بيع": 6.0},
    ...
    if menu == "💎 منصة البيع السريع":
        st.markdown("<h1>🛒 فاتورة المبيعات الملكية</h1>", unsafe_style_html=True)
        
        m1, m2 = st.columns(2)
        with m1:
            st.markdown(f"<div class='metric-card'><h3>📈 أرباح اليوم</h3><h2>{st.session_state.daily_profit:.2f} ₪</h2></div>", unsafe_style_html=True)
        with m2:
            st.markdown(f"<div class='metric-card'><h3>📦 عدد الأصناف</h3><h2>{len(st.session_state.inventory)} صنف</h2></div>", unsafe_style_html=True)
        
        bill_items = []
        st.write("### اختر المشتريات:")
        
        # ترويسة الجدول
        cols_header = st.columns([0.5, 2, 2, 3])
        cols_header[0].write("**إختر**")
        cols_header[1].write("**الصنف**")
        cols_header[2].write("**النوع**")
        cols_header[3].write("**القيمة**")
        ...
        for item in list(st.session_state.inventory.keys()):
            ...
            with c4: v = st.number_input("", min_value=0.0, step=0.1, key=f"v_{item}", label_visibility="collapsed")
        ...
        if bill_items:
            ...
            st.markdown(f"<h2 style='text-align:left; color:#1e4d2b;'>💰 إجمالي الفاتورة: {total_bill:.2f} شيكل</h2>", unsafe_style_html=True)
    ...
    # 2. إضافة صنف جديد
        ...
        st.markdown("<h1>📦 إضافة صنف جديد</h1>", unsafe_style_html=True)
        with st.form("add_form"):
            n = st.text_input("اسم الصنف")
            q = st.number_input("الكمية المتوفرة", min_value=0.0)
            b = st.number_input("سعر الشراء (للكيلو)", min_value=0.0)
            s = st.number_input("سعر البيع (للكيلو)", min_value=0.0)
    ...
    # 3. تعديل الأسعار
    elif menu == "🛠️ تعديل الأسعار":
        st.markdown("<h1>🛠️ تعديل الكميات والأسعار</h1>", unsafe_style_html=True)
        it = st.selectbox("اختر الصنف", list(st.session_state.inventory.keys()))
        extra_q = st.number_input("إضافة كمية جديدة للمخزن", min_value=0.0)
        new_p = st.number_input("تعديل سعر البيع الحالي", value=st.session_state.inventory[it]["بيع"])
        if st.button("تحديث"):
            st.session_state.inventory[it]["كمية"] += extra_q
            st.session_state.inventory[it]["بيع"] = new_p
            st.success("تم التحديث")

    # 4. التوالف
    elif menu == "🍂 قسم التوالف والخسائر":
        st.markdown("<h1>🍂 تسجيل التوالف</h1>", unsafe_style_html=True)
