...
if menu == "🛒 نقطة البيع":
    ...
    for idx, (it, data) in enumerate(items.items()):
        with cols[idx % 4]:
            ...
            # تم تعديل القيمة الافتراضية لتكون فارغة
            val = st.number_input(f"الكمية ({it})", key=f"v_{it}", min_value=0.0, step=0.1, value=None)
            if val and val > 0:
            ...
            if (f_cat == "الكل" or item_cat == f_cat) and (search_st.lower() in it.lower()):
                ...
                with cols[display_idx % 3]:
                    ...
                    with st.expander(f"⚙️ جرد {it}"):
                        new_q = st.number_input("الكمية الفعلية", value=None, key=f"inv_q_{it}", placeholder="ادخل الكمية")
    ...
    # تم الإبقاء على كود التقارير وحذف الفاتورة كما هو دون تغيير
    ...
    if not df_sales.empty:
        ...
        if st.button(f"🗑️ حذف آخر فاتورة (رقم: {last_bill_id})", use_container_width=True):
            ...
            if sync_to_google(): st.success("تم التراجع بنجاح"); st.rerun()
    ...
    with st.form("exp_form"):
        r = st.text_input("البيان"); a = st.number_input("المبلغ (₪)", min_value=0.0, value=None)
