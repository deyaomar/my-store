...
if menu == "🛒 نقطة البيع":
    ...
    # (تم الإبقاء على كود التقارير الاحترافي الذي طلبته سابقاً مدمجاً هنا)
    ...
    if not st.session_state.sales_df.empty:
        ...
        with col1: st.markdown(f"<div style='background: linear-gradient(135deg, #27ae60, #2ecc71); padding: 20px; border-radius: 15px; color: white; text-align: center;'>مبيعات اليوم<br><h2>{format_num(today_data['amount'].sum())}</h2></div>", unsafe_allow_html=True)
        with col2: st.markdown(f"<div style='background: linear-gradient(135deg, #2980b9, #3498db); padding: 20px; border-radius: 15px; color: white; text-align: center;'>أرباح اليوم<br><h2>{format_num(today_data['profit'].sum())}</h2></div>", unsafe_allow_html=True)
        with col3: st.markdown(f"<div style='background: linear-gradient(135deg, #8e44ad, #9b59b6); padding: 20px; border-radius: 15px; color: white; text-align: center;'>مبيعات الأسبوع<br><h2>{format_num(week_data['amount'].sum())}</h2></div>", unsafe_allow_html=True)
        with col4: st.markdown(f"<div style='background: linear-gradient(135deg, #f39c12, #f1c40f); padding: 20px; border-radius: 15px; color: white; text-align: center;'>أرباح الأسبوع<br><h2>{format_num(week_data['profit'].sum())}</h2></div>", unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        daily_summary = week_data.groupby(week_data['date'].dt.date).agg({'amount': 'sum', 'profit': 'sum'}).reset_index()
        st.table(daily_summary.sort_values(by='date', ascending=False))
