...
if st.session_state.user_role == "admin":
    ...
    if menu == "📊 التقارير العامة":
        ...
        # هنا تظهر ملخصات كل الفروع لأبو عمر
        st.write("إحصائيات شاملة للنظام...")
...
# --- 6. شاشة مسؤول الفرع (الكود الذي أرسلته) ---
else:
    ...
    if menu == "🛒 نقطة البيع":
        ...
        st.markdown("<h1 class='main-title'>📦 إدارة مخزن الفرع</h1>", unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(my_inv))

    elif menu == "📊 التقارير المالية":
        st.markdown("<h1 class='main-title'>📊 تقارير أداء الفرع</h1>", unsafe_allow_html=True)
        # حسابات الأرباح ورأس المال للفرع فقط
        inv_df = pd.DataFrame(my_inv)
        if not inv_df.empty:
            capital = (inv_df['شراء'] * inv_df['كمية']).sum()
            st.metric("إجمالي رأس مال الفرع", f"{format_num(capital)} ₪")
