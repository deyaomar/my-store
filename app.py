...
# ضمان بقاء قسم سجائر ثابت ومحمي
...
if menu == "⚙️ إدارة الأصناف":
    ...
    with tab_add:
        # هنا البرمجة الخاصة بقسم السجائر
        cat_sel = st.selectbox("اختر القسم لتحديد نظام الإدخال:", st.session_state.categories)
        
        with st.form("form_add_special", clear_on_submit=True):
            n = st.text_input("اسم الصنف")
            
            if cat_sel == "سجائر":
                st.warning("🚬 نظام السجائر (ثابت): أدخل العلب والفرط")
                c1, c2 = st.columns(2)
                q_box = c1.text_input("عدد العلب الكاملة", value="0")
                q_singles = c2.text_input("عدد السجائر الفرط", value="0")
                b = st.text_input("تكلفة العلبة الواحدة")
                s = st.text_input("سعر بيع العلبة")
                sub_p = st.text_input("سعر بيع السيجارة الواحدة")
            else:
                q_box = st.text_input("الكمية")
                q_singles = "0"
            ...
            if st.form_submit_button("إضافة للمخزن"):
                if n:
                    final_q = clean_num(q_box) + (clean_num(q_singles) / 20)
                    st.session_state.inventory.append({
                        "item": n, "قسم": cat_sel, "شراء": clean_num(b), 
                        "بيع": clean_num(s), "كمية": final_q, 
                        "branch": st.session_state.my_branch, "سعر_القطعة": clean_num(sub_p)
                    })
                    auto_save(); st.success(f"تمت إضافة {n}"); st.rerun()

    with tab_manage:
        br_data = [i for i in st.session_state.inventory if i.get('branch') == st.session_state.my_branch]
        if br_data:
            df_edit = st.data_editor(pd.DataFrame(br_data)[['item', 'قسم', 'شراء', 'بيع', 'سعر_القطعة', 'كمية']], use_container_width=True)
            if st.button("حفظ التعديلات"):
                new_inv = [i for i in st.session_state.inventory if i.get('branch') != st.session_state.my_branch]
                for _, row in df_edit.iterrows():
                    new_inv.append({**row.to_dict(), "branch": st.session_state.my_branch})
                st.session_state.inventory = new_inv
                auto_save(); st.success("تم التحديث"); st.rerun()

    with tab_cats:
        with st.form("cat_form"):
            nc = st.text_input("اسم قسم جديد")
...
# --- شاشة البيع ---
    ...
    for it in my_inv:
        if not search or search.lower() in it['item'].lower():
            with st.container():
                ...
                c1.write(f"**{it['item']}**")
                mode = c2.selectbox("النوع", ["علبة/وحدة", "فرط/تجزئة"] if it.get('سعر_القطعة', 0) > 0 else ["وحدة"], key=f"m_{it['item']}")
                ...
                if val > 0:
                    if mode == "فرط/تجزئة":
                        ...
                        prof = val - (cost * (val / it['سعر_القطعة']))
                    else:
                        qty = val / it['بيع']; prof = (it['بيع'] - it['شراء']) * qty
                    bill_items.append({"item": it['item'], "qty": qty, "amount": val, "profit": prof})
    if st.button("🚀 اعتماد البيع") and bill_items:
        for e in bill_items:
            ...
            st.session_state.sales_df = pd.concat([st.session_state.sales_df, pd.DataFrame([{'date': datetime.now().strftime("%Y-%m-%d %H:%M"), 'item': e['item'], 'amount': e['amount'], 'profit': e['profit'], 'method': 'نقداً', 'customer_name': 'عام', 'customer_phone': '', 'bill_id': str(uuid.uuid4())[:8], 'branch': st.session_state.my_branch}])], ignore_index=True)
...
# باقي الأقسام
    ...
    st.metric("مبيعاتك اليوم", f"{format_num(s_df['amount'].sum())} ₪")
    st.dataframe(s_df)
