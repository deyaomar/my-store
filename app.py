...
# 3. الاتصال بقاعدة البيانات والمزامنة
...
if menu == "🛒 نقطة البيع":
    ...
    if not st.session_state.show_customer_form:
        ...
        if st.button("✅ تأكيد وحفظ"):
            ...
            for e in st.session_state.current_bill_items:
                ...
                new_sales.append({'date': datetime.now().strftime("%Y-%m-%d %H:%M"), 'item': e['item'], 'amount': e['amount'], 'profit': e['profit'], 'method': e['method'], 'customer_name': c_n, 'phone': c_p, 'bill_id': bid})
    ...
    with st.expander("⚠️ تسجيل بضاعة تالفة (فاقد)"):
        with st.form("waste_form"):
            ...
            if st.form_submit_button("تسجيل التالف وخصمه من المخزن"):
                if 0 < w_qty <= st.session_state.inventory[w_item]['كمية']:
                    ...
                    sync_to_google()
                    st.success(f"تم تسجيل {w_qty} من {w_item} كتالف")
                    st.rerun()
                else:
                    st.error("الكمية غير صحيحة أو أكبر من المتوفر")
            ...
            if (f_cat == "الكل" or item_cat == f_cat) and (search_st.lower() in it.lower()):
                ...
                with cols[display_idx % 3]:
                    ...
                    with st.expander(f"⚙️ جرد {it}"):
                        new_q = st.number_input("الكمية الفعلية", key=f"inv_q_{it}")
    ...
    if not df_exp.empty: df_exp['date'] = pd.to_datetime(df_exp['date']).dt.normalize()
        
    df_waste = st.session_state.waste_df.copy()
    if not df_waste.empty: df_waste['date'] = pd.to_datetime(df_waste['date']).dt.normalize()
    ...
    c1.metric("رأس المال في المخزن", f"{format_num(current_stock_cap)} ₪")
    c2.metric("مبيعات اليوم", f"{format_num(t_sales)} ₪")
    c3.metric("صافي ربح اليوم", f"{format_num(t_net_profit)} ₪")

    st.markdown("---")
    st.write("### 📝 سجل مبيعات اليوم")
    st.dataframe(df_sales[df_sales['date'] == today], use_container_width=True)
    ...
    with st.expander("➕ إضافة مصروف جديد", expanded=True):
        with st.form("new_exp_form"):
            ...
            reason = col1.text_input("البيان")
            amount = col2.number_input("المبلغ", min_value=0, step=1)
    ...
    if not st.session_state.expenses_df.empty:
        st.dataframe(st.session_state.expenses_df.iloc[::-1], use_container_width=True)
