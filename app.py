...
# 4. تحميل البيانات (أضفنا أعمدة جديدة للسيلز)
...
if menu == "🛒 نقطة البيع":
    ...
    for idx, (it, data) in enumerate(items.items()):
        with cols[idx % 4]:
            ...
            val = st.number_input("الكمية", key=f"sell_{it}", min_value=0.0, value=None, placeholder="0")
            if val is not None and val > 0:
                temp_bill.append({'item': it, 'qty': val, 'amount': val * data['بيع'], 'profit': (data['بيع'] - data['شراء']) * val})
    
    # --- قسم تأكيد الفاتورة المطور ---
    if temp_bill:
        st.markdown("<div class='bill-section'>", unsafe_allow_html=True)
        st.subheader("📝 تفاصيل الفاتورة والزبون")
        
        col_pay1, col_pay2 = st.columns(2)
        with col_pay1:
            pay_method = st.radio("💰 طريقة الدفع:", ["نقدي", "تطبيق"], horizontal=True)
            cust_name = st.text_input("👤 اسم الزبون (اختياري)", placeholder="مثال: أحمد محمد")
        with col_pay2:
            cust_phone = st.text_input("📞 رقم الجوال (اختياري)", placeholder="05xxxxxxx")
            total_sum = sum(item['amount'] for item in temp_bill)
            st.markdown(f"### إجمالي الحساب: <span style='color:red;'>{format_num(total_sum)} ₪</span>", unsafe_allow_html=True)

        if st.button("✅ إتمام العملية وحفظ", use_container_width=True):
            bid = str(uuid.uuid4())[:8]
            final_name = cust_name if cust_name else "زبون محل"
            final_phone = cust_phone if cust_phone else "-"
            ...
            sync_to_google()
            st.success(f"تم تسجيل العملية بنجاح! رقم الفاتورة: {bid}")
            st.rerun()
        ...
        for it, data in st.session_state.inventory.items():
            ...
            if (f_cat == "الكل" or item_cat == f_cat) and (search_st.lower() in it.lower()):
                with cols[display_idx % 3]:
                    ...
                    st.markdown(f"<div class='stock-card' style='border-top: 6px solid {card_color};'><h3>{it}</h3><p>المتبقي: {data['كمية']}</p><h4>{data['بيع']} ₪</h4></div>", unsafe_allow_html=True)
                    with st.expander(f"⚙️ جرد {it}"):
                        new_q = st.number_input("الكمية الفعلية", value=None, key=f"inv_q_{it}")
                        if st.button("تحديث", key=f"inv_btn_{it}"):
                            if new_q is not None:
                                st.session_state.inventory[it]['كمية'] = new_q; sync_to_google(); st.rerun()
                display_idx += 1

elif menu == "📊 التقارير المالية":
    st.markdown("<h1 class='main-title'>📊 سجل المبيعات والبيانات</h1>", unsafe_allow_html=True)
    ...
    if not df_sales.empty:
        ...
        if st.button(f"🗑️ حذف آخر فاتورة (رقم: {last_bill_id})", use_container_width=True):
            ...
            sync_to_google(); st.success("تم التراجع عن الفاتورة بنجاح!"); st.rerun()

elif menu == "💸 المصروفات":
    st.markdown("<h1 class='main-title'>💸 إدارة المصروفات</h1>", unsafe_allow_html=True)
    with st.form("exp_form"):
        r = st.text_input("البيان")
        a = st.number_input("المبلغ", min_value=0.0, value=None)
    ...
    with t2:
        with st.form("add_form"):
            ...
            b = st.number_input("شراء", min_value=0.0, value=None)
            s = st.number_input("بيع", min_value=0.0, value=None)
