...
# دالة لتحويل الأرقام للنص بدون أصفار زائدة (للعرض فقط)
def format_num(val):
...
# 3. التصميم (CSS) - الحفاظ على تنسيقك المفضل
st.markdown("""
    <style>
        ...
        if st.session_state.last_report:
            ...
            if st.button("✅ تأكيد البيع والحفظ", type="primary"):
                if bill_items:
                    ...
                    st.session_state.last_report = f"<div style='border:2px solid green; padding:10px;'><h3>تم الحفظ بنجاح!</h3><h4>الإجمالي: {format_num(total)} ₪</h4></div>"
                    auto_save(); st.rerun()

    # --- 2. إدارة الأصناف ---
    elif menu == "⚙️ إدارة الأصناف":
        st.markdown("<h1 class='main-title'>⚙️ الإعدادات</h1>", unsafe_allow_html=True)
        tab1, tab2 = st.tabs(["🆕 إضافة صنف", "✏️ تعديل/حذف"])
        with tab1:
            with st.form("add"):
        ...
        with tab2:
            ...
            if edit_item:
                ...
                new_sell = st.text_input("سعر البيع", value=format_num(d['بيع']))
                new_qty = st.text_input("الكمية", value=format_num(d['كمية']))
                if st.button("حفظ التعديل"):
                    st.session_state.inventory[edit_item]['بيع'] = clean_num(new_sell)
                    st.session_state.inventory[edit_item]['كمية'] = clean_num(new_qty)
