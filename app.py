...
def format_num(val):
    try:
        ...
        return str(round(val, 3)) # زيادة الدقة لغرض الجرامات والسيجار
...
if 'inventory' not in st.session_state:
    # تم إضافة أعمدة نوع_الوحدة وسعر_القطعة (التجزئة)
...
# --- قسم إدارة الأصناف المحدث لدعم التجزئة ---
if menu == "⚙️ إدارة الأصناف":
    ...
    with t_add:
        with st.form("admin_add_i", clear_on_submit=True):
            n = st.text_input("اسم الصنف (مثلاً: دخان مالبورو)")
            cat = st.selectbox("القسم", st.session_state.categories)
            u_type = st.selectbox("وحدة القياس الكبرى", ["علبة", "كيلو", "قطعة"])
            
            c1, c2 = st.columns(2)
            b = c1.text_input(f"سعر شراء الـ ({u_type})")
            s = c2.text_input(f"سعر بيع الـ ({u_type})")
            
            st.markdown("---")
            st.write("🎯 **خيار البيع بالتجزئة (اختياري)**")
            has_sub = st.checkbox("تفعيل سعر (السيجارة / الغرام)")
            sub_p = st.text_input("سعر بيع الوحدة الصغيرة (مثلاً 5 شيكل للسيجارة)", value="0.0")
            
            q = st.text_input(f"الكمية المتوفرة بـ ({u_type})")
            
            if st.form_submit_button("➕ تنفيذ الإضافة"):
                if n:
                    st.session_state.inventory.append({
                        "item": n, "قسم": cat, "شراء": clean_num(b), "بيع": clean_num(s), 
                        "كمية": clean_num(q), "branch": target_branch,
                        "نوع_الوحدة": u_type, "سعر_القطعة": clean_num(sub_p) if has_sub else 0
                    })
                    auto_save(); st.success("✅ تم الإضافة بنجاح"); st.rerun()
    ...
    with t_manage:
        ...
        if branch_data:
            ...
            edited_df = st.data_editor(
                df_branch[['item', 'قسم', 'نوع_الوحدة', 'شراء', 'بيع', 'سعر_القطعة', 'كمية']],
            ...
            if st.button("💾 حفظ كافة التغييرات للفرع"):
                ...
                for _, row in edited_df.iterrows():
                    new_inventory.append({**row.to_dict(), "branch": target_branch})
                st.session_state.inventory = new_inventory
                auto_save(); st.success("✅ تم التحديث"); st.rerun()

    with t_cats:
        st.subheader("التحكم في الأقسام")
...
# --- قسم نقطة البيع المحدث لدعم التجزئة ---
    ...
    if st.session_state.show_cust_fields:
        with st.expander("✅ تم اعتماد الفاتورة!", expanded=True):
        ...
        st.session_state.p_method = st.radio("الدفع", ["تطبيق", "نقداً"], horizontal=True)
        search_q = st.text_input("🔍 بحث سـريع...")
        bill_items = []
        ...
        for it in my_inv:
            if not search_q or search_q.lower() in it['item'].lower():
                with st.container():
                    ...
                    col1.write(f"**{it['item']}** ({it.get('نوع_الوحدة', 'قطعة')})")
                    
                    # اختيار نوع البيع (جملة الوحدة أو تجزئة)
                    opts = [f"بالـ {it.get('نوع_الوحدة', 'قطعة')}"]
                    if it.get('سعر_القطعة', 0) > 0:
                        sub_name = "سيجارة" if it['نوع_الوحدة'] == "علبة" else "غرام/قطعة"
                        opts.append(f"بالـ {sub_name}")
                    
                    sale_mode = col2.selectbox("طريقة البيع", opts, key=f"mode_{it['item']}")
                    val = clean_num(col3.text_input("المبلغ ₪", key=f"v_{it['item']}"))
                    
                    if val > 0:
                        if "بالـ العلبة" in sale_mode or "بالـ كيلو" in sale_mode or "بالـ قطعة" in sale_mode:
                            qty = val / it['بيع']
                            profit = (it['بيع'] - it['شراء']) * qty
                        else:
                            # حساب التجزئة (20 سيجارة للعلبة أو 1000 جرام للكيلو)
                            factor = 20 if it['نوع_الوحدة'] == "علبة" else 1000
                            qty = (val / it['سعر_القطعة']) / factor
                            cost_small = it['شراء'] / factor
                            profit = val - (cost_small * (val / it['سعر_القطعة']))
        ...
        if st.button("🚀 إتمام البيع") and bill_items:
            ...
            st.session_state.current_bill_id = b_id; auto_save(); st.session_state.show_cust_fields = True; st.rerun()
