...
if menu == "⚙️ إدارة الأصناف":
    ...
    with t_add:
        with st.form("admin_add_i", clear_on_submit=True):
            ...
            b = st.text_input("سعر التكلفة (شراء)", value="0")
            s = st.text_input("سعر البيع", value="0")
            q = st.text_input("الكمية", value="0")
                ...
                for idx, it in enumerate(items):
                    with grid[idx % 3]:
                        with st.container(border=True):
                            ...
                            val = st.number_input(f"المبلغ - {it['item']}", min_value=0.0, value=0.0, step=1.0, key=f"pos_{it['item']}_{idx}", label_visibility="collapsed")
    ...
    with st.form("br"):
        ...
        if st.form_submit_button("حفظ"):
            new_branch = pd.DataFrame([{'branch_name':bn,'user_name':un,'password':pw, 'role': 'shop'}])
            pd.concat([pd.read_csv(get_db_path()), new_branch]).to_csv(get_db_path(), index=False)
    ...
    with c1: st.markdown(f'<div class="rep-card" style="border-top-color: #3498db;"><div class="rep-label">💰 إجمالي المبيعات</div><div class="rep-value">{format_num(total_sales)} ₪</div></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="rep-card" style="border-top-color: #27ae60;"><div class="rep-label">📈 صافي الأرباح</div><div class="rep-value">{format_num(total_profit)} ₪</div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="rep-card" style="border-top-color: #e74c3c;"><div class="rep-label">💸 إجمالي المصاريف</div><div class="rep-value">{format_num(total_exp)} ₪</div></div>', unsafe_allow_html=True)
    with c4: 
        color = "#27ae60" if net_total >= 0 else "#e74c3c"
        st.markdown(f'<div class="rep-card" style="border-top-color: {color};"><div class="rep-label">⚖️ المتبقي النهائي</div><div class="rep-value" style="color: {color};">{format_num(net_total)} ₪</div></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["📄 تفاصيل المبيعات", "📉 تفاصيل المصاريف"])
    with t1: st.dataframe(s_df.sort_values(by='date', ascending=False), use_container_width=True)
    with t2: st.dataframe(e_df.sort_values(by='date', ascending=False), use_container_width=True)
    ...
    if not my_inv:
        ...
        with st.container(border=True):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("عدد الأصناف", f"{len(df_inv)}")
            c2.metric("رأس المال (شراء)", f"{format_num((df_inv['كمية'] * df_inv['شراء']).sum())} ₪")
            c3.metric("قيمة البيع", f"{format_num((df_inv['كمية'] * df_inv['بيع']).sum())} ₪")
            c4.metric("الربح المتوقع", f"{format_num((df_inv['كمية'] * df_inv['بيع']).sum() - (df_inv['كمية'] * df_inv['شراء']).sum())} ₪")
        st.markdown("---")
        jard_updates = []
        h1, h2, h3, h4, h5 = st.columns([2.5, 1, 1, 1.5, 1.5])
        h1.write("**الصنف والقسم**"); h2.write("**شراء / بيع**"); h3.write("**النظام**"); h4.write("**الجرد الفعلي**"); h5.write("**الحالة / الفرق**")
        for idx, it in enumerate(my_inv):
            with st.container(border=True):
                ...
                diff = actual - it['كمية']
                status_color = "#16a34a" if diff == 0 else "#dc2626" if diff < 0 else "#2563eb"
                status_text = "✅ مطابق" if diff == 0 else f"⚠️ عجز ({format_num(diff)})" if diff < 0 else f"➕ زيادة (+{format_num(diff)})"
                col5.markdown(f"<div style='background:{status_color}; color:white; padding:8px; border-radius:10px; text-align:center; font-size:0.9em; font-weight:bold;'>{status_text}</div>", unsafe_allow_html=True)
                if diff != 0: jard_updates.append({'item': it['item'], 'new_qty': actual, 'diff': diff})
        if jard_updates:
            if st.button("💾 اعتماد نتائج الجرد", use_container_width=True, type="primary"):
                for up in jard_updates:
                    for i, inv_item in enumerate(st.session_state.inventory):
                        if inv_item['item'] == up['item'] and inv_item['branch'] == st.session_state.my_branch:
                            ...
                            adj_log = {'date': datetime.now().strftime("%Y-%m-%d %H:%M"), 'item': up['item'], 'diff': up['diff'], 'branch': st.session_state.my_branch}
