...
st.markdown("""
    ...
    .stButton>button:hover { background-color: #219150; border: none; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. إدارة البيانات (الأساس المتين) ---
...
if 'db' not in st.session_state:
    ...
    st.session_state.cats = pd.read_csv('cats.csv')['name'].tolist() if os.path.exists('cats.csv') else ["خضار وفواكه", "مكسرات", "أخرى"]
...
if 'auth' not in st.session_state:
    ...
    with col:
        ...
        if st.button("دخول"):
            ...
            else: st.error("خطأ في كلمة المرور، حاول مجدداً")
else:
    # --- 4. القائمة الجانبية المنظمة ---
    st.sidebar.markdown(f"<div style='text-align:center'><h2 style='color:#27ae60;'>أبو عمر 👋</h2><p>نظام 2026 المستقر</p></div>", unsafe_allow_html=True)
    st.sidebar.divider()
    menu = st.sidebar.radio("التنقل السريع:", 
        ["🛒 نقطة البيع (POS)", "📊 التقارير والأرباح", "📦 إدارة المخزن", "⚖️ تنفيذ جرد", "💸 المصروفات", "⚙️ الإعدادات"])
    ...
    if menu == "🛒 نقطة البيع (POS)":
        ...
        with c_p1:
            search = st.text_input("🔍 ابحث عن صنف لبيعه (بندورة، لوز، إلخ)...")
        ...
        if display_items.empty:
            st.warning("لا توجد أصناف تطابق بحثك.")
        else:
            for idx, row in display_items.iterrows():
                with st.container():
                    ...
                    unit = col_u.radio("البيع بـ", ["شيكل", "وزن"], key=f"u_{idx}", horizontal=True)
        ...
        if st.button("💾 إتمام العملية وحفظ الفاتورة", type="primary"):
            if cart:
                ...
                save_db(); st.success(f"✅ تم الحفظ! فاتورة رقم: {bill_id}"); st.rerun()

    # --- 📊 2. التقارير والأرباح (من وإلى) ---
        ...
        t_type = col_t1.selectbox("فترة التقرير", ["اليوم", "آخر 7 أيام", "تاريخ مخصص"])
        
        start_d = datetime.now().date()
        end_d = datetime.now().date()
        if t_type == "آخر 7 أيام": start_d -= timedelta(days=7)
        elif t_type == "تاريخ مخصص":
            start_d = col_t2.date_input("من", datetime.now().date() - timedelta(days=30))
            end_d = col_t3.date_input("إلى", datetime.now().date())
        ...
        met1.metric("إجمالي المبيعات", f"{rev:,.1f} ₪")
        met2.metric("المصروفات", f"{exps:,.1f} ₪")
        met3.metric("خسائر الجرد", f"{adj_loss:,.1f} ₪")
        met4.metric("صافي الربح", f"{net:,.1f} ₪", delta=f"{prof:,.1f} ربح بيع", delta_color="normal")

        st.divider()
        st.subheader("📋 سجل الفواتير المفصل")
        if not s_f.empty:
            st.dataframe(s_f[['التاريخ', 'الصنف', 'المبلغ', 'الطريقة', 'الزبون', 'رقم_الفاتورة']], use_container_width=True)

    # --- 📦 3. إدارة المخزن ---
    elif menu == "📦 إدارة المخزن":
        st.markdown("<div class='main-header'><h1>📦 رصيد المخزن الحالي</h1></div>", unsafe_allow_html=True)
    ...
    # --- ⚖️ 4. تنفيذ جرد ---
        ...
        if st.button("💾 اعتماد الجرد وتصحيح العجز", type="primary"):
            ...
            save_db(); st.success("تم تحديث المخزن وحساب الفوارق المالية."); st.rerun()

    # --- 💸 5. المصروفات ---
    elif menu == "💸 المصروفات":
        st.markdown("<div class='main-header'><h1>💸 تسجيل مصروفات</h1></div>", unsafe_allow_html=True)
        with st.form("exp_f"):
            reason = st.text_input("بيان المصروف (كهرباء، أكياس، عمال...)")
            ...
            if st.form_submit_button("حفظ المصروف"):
                ...
                save_db(); st.rerun()
        st.table(st.session_state.db['exp'].sort_index(ascending=False).head(10))

    # --- ⚙️ 6. الإعدادات ---
    elif menu == "⚙️ الإعدادات":
        st.markdown("<div class='main-header'><h1>⚙️ إدارة الأصناف والأقسام</h1></div>", unsafe_allow_html=True)
        with st.expander("➕ إضافة صنف جديد"):
            with st.form("add_i"):
                ...
                q = col3.text_input("الكمية الحالية")
