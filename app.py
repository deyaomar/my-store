...
st.markdown("""
    ...
    .main-title { 
        color: #1a1a1a; font-weight: 900; font-size: 24px; 
        border-right: 8px solid #27ae60; padding-right: 15px; margin-bottom: 20px; 
    }
    ...
    /* جعل الأزرار سهلة الضغط على اللمس */
    .stButton>button { width: 100% !important; border-radius: 10px; height: 3.5em; }
    </style>
...
if menu == "🛒 نقطة البيع":
    ...
    # في الموبايل يفضل وضع العناصر تحت بعضها
    ...
    temp_bill = []
    # عرض الأصناف بشكل قائمة تحت بعضها للموبايل
    for it, data in items.items():
        with st.container():
            col_info, col_input = st.columns([2, 1])
            col_info.markdown(f"**{it}** \n<span style='color:green;'>{data['بيع']} ₪</span> | <small>متوفر: {data['كمية']}</small>", unsafe_allow_html=True)
            val = col_input.number_input("الكمية", key=f"v_{it}", min_value=0.0, step=1.0)
            if val > 0:
    ...
    if st.session_state.inventory:
        ...
        st.markdown(f"<div class='report-card'><h5>قيمة رأس المال الحالي</h5><h2>{format_num(stock_value)} ₪</h2></div>", unsafe_allow_html=True)
        
        f_cat = st.selectbox("📂 تصفية حسب القسم", ["الكل"] + st.session_state.CATEGORIES)
        ...
        for it, data in st.session_state.inventory.items():
            ...
            if (f_cat == "الكل" or item_cat == f_cat) and (search_st.lower() in it.lower()):
                ...
                with st.expander(f"📦 {it} - المتبقي: {data['كمية']}"):
                    ...
                    new_q = st.number_input("تعديل الكمية", value=float(data['كمية']), key=f"inv_q_{it}")
    ...
    st.markdown("<h1 class='main-title'>📊 تقارير الأداء</h1>", unsafe_allow_html=True)
    # المنطق المالي (تم تنظيفه ليعمل بسلاسة)
